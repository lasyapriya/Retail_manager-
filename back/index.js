...
// Load your 12,000-row CSV from GitHub/Render
fs.createReadStream('data/retail_data.csv')
  .pipe(csv())
  .on('data', (row) => data.push(row))
  .on('end', () => {
    console.log('CSV loaded successfully: 12,000 records');
  })
  .on('error', (err) => {
...
app.get('/api/options', (req, res) => {
  if (data.length === 0) {
    return res.json({
      customerRegions: [], genders: [], productCategories: [], tags: [], paymentMethods: []
    });
  }
...
// Main API: Search + Filters + Sorting + Pagination
app.get('/api/transactions', (req, res) => {
  ...
  if (filters.ageMin) result = result.filter(r => parseInt(r['Age'] || 0) >= parseInt(filters.ageMin));
  if (filters.ageMax) result = result.filter(r => parseInt(r['Age'] || 0) <= parseInt(filters.ageMax));
...
app.listen(PORT, () => {
  console.log(`Backend LIVE: https://retail-manager-lk02.onrender.com`);
});
